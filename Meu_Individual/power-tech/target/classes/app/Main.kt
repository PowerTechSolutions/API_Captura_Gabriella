...
open class Main {
    companion object {
            ...
            if (usuario_repositorio.autenticar(Cpf)){
                ...
                for (servico in servicos){
                    ...
                    when(apelido){
                        ...
                        "USB" -> funcoes.add("U")
                        "DISCO" -> funcoes.add("D")
                        "PROCESSOS" -> funcoes.add("P")
                        "DESEMPENHO" -> funcoes.add("DP")
                        else -> {
                            funcoes.add("CR")
                        }
        ...
        fun Captura(funcoes:MutableList<String>,servicos: MutableList<ServicosMonitorados>, maquinaEscolhida: Int, dataAtual: LocalDateTime){
            ...
            var capturaPROCESSOS= 0
            var capturaDESEMPENHO= 0
            ...
            for (servico in funcoes){
                when(servico){
                    ...
                    "D" -> capturaDISCO = 1
                    "U" -> capturaUSB = 1
                    "P" -> capturaPROCESSOS = 1
                    "DP" -> capturaDESEMPENHO = 1
                    "CR" -> capturaCPU_RAM = 1
                }
            }
            ...
            var verificacaoProc= false
            var verificacaoDesemp= false

            while(true){

                if (capturaDISCO == 1  || capturaDESEMPENHO ==1)){
...
//                        verificacaoSemanal += 1
                ...
                if (capturaREDE == 1 || capturaJANELAS == 1 ||
                    ...
                    if (verificacaoDiaria == 0){
                        ...
                        if (capturaPROCESSOS == 1){
                            if (verificacaoDesemp){
                                var dataJanelas = repositorio.buscarDataJanela(maquinaEscolhida)
                                if (dataJanelas.dayOfMonth+1 ==dataAtual.dayOfMonth){
                                    captura.pegarJanelas(maquinaEscolhida)
                                }
