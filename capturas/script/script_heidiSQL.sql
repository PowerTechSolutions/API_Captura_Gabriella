...
CREATE TABLE IF NOT EXISTS Alertas(
    ...
    FKUnidade_negocio INT,
		...
		produtividade_maquina VARCHAR(60),
...
INSERT into Usuario_Dashboard VALUES
(NULL,'gabi','gabi@teste.com','48372073840','18080226',1,2);
...
WHERE IDUsuario = 3 
ORDER BY dthora_captura DESC
LIMIT 1;

SELECT ROUND(SUM(tempo_user)/ 60,2) AS tempo_em_minutos FROM Processos JOIN 
                Maquinas ON fkmaquina_processo = IDMaquina
        WHERE IDMaquina = 4 AND Processos.nome IN ('tradingscreen.exe', 'energyquant.exe', 
        'bloombergbash.exe', 'eikon.exe', 'powermarket.exe', 'calypso.exe')
        LIMIT 1;
...
JOIN Maquinas ON fkmaquina_processo = IDMaquina
JOIN Usuario_Dashboard ON IDUsuario = FKFuncionario
WHERE IDUsuario = 3 AND Processos.nome NOT IN ('tradingscreen.exe', 'energyquant.exe', 'bloombergbash.exe', 'eikon.exe', 'powermarket.exe', 'calypso.exe')
ORDER BY dthora_captura DESC
LIMIT 1;

SELECT produtividade_maquina FROM Processos WHERE fkmaquina_processo=3;

SELECT * FROM Processos JOIN Maquinas ON fkmaquina_processo= IDMaquina 
JOIN Usuario_Dashboard ON IDUsuario = FKFuncionario WHERE IDUsuario=2

SELECT ROUND(SUM(tempo_user)/ 60,2) AS tempo_em_minutos FROM Processos JOIN Maquinas ON fkmaquina_processo = IDMaquina
JOIN Usuario_Dashboard ON IDUsuario = FKFuncionario
WHERE IDUsuario = 3 AND Processos.nome IN ('tradingscreen.exe', 'energyquant.exe', 'bloombergbash.exe', 'eikon.exe', 'powermarket.exe', 'calypso.exe')
